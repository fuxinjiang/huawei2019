...
def save_lgbmodel(model, file_path):
	if not os.path.exists(file_path):
		os.makedirs(file_path)
	model.save_model(file_path + '/lgb_dart.model')
#定义加权的f1值
...
if __name__ == "__main__":
	...
	train_x = sparse.hstack((X_bhv_log, X_bsc1, X_bsc2, X_act, X_app, train_sparse_matrix_npz))
	...
	x_test = sparse.hstack((X_bhv_log_test, X_bsc1_test, X_bsc2_test, X_act_test, X_app_test, test_sparse_matrix_npz))
	...
	'''
	#lgbtm模型
	config_lgb = json.load(codecs.open('lgb_config_dart.json', 'r', 'utf-8'))
	...
	test_prediction_label = np.argmax(lgb_model.predict(x_test), axis=1)
	...
	data_final.to_csv('submission_1w_dimension_dart.csv', index=False)
