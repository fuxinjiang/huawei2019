...
def data_pre():
    ...
    names_categ = 'gender city prodName ramCapacity romCapacity color fontSize ct carrier os'.split()
    # 离散属性的值空间个数
    ...
    #先对'ct'和'carrier'进行dummy变量操作
    dummy_list = ['ct', 'carrier']
    for dummy_name in dummy_list:
        dummies = pd.get_dummies(user_basic_info.loc[:, dummy_name], prefix=dummy_name)
        user_basic_info = pd.concat([user_basic_info, dummies], axis=1)
    # 对prodName、city、还有color进行热度编码
    origin_cate_feature = ['prodName', 'city', 'color']
    count_feature_list = []
    def feature_count(data, features=[]):
        #进行特征组合的时候，排除两个特征相同的情况
        ...
        return data
    print("开始对单特征进行编码!")
    for i in origin_cate_feature:
        user_basic_info = feature_count(user_basic_info, [i])
    print("开始进行交叉特征的编码!")

    for i in names_categ:
        for j in names_categ:
            print(i)
            user_basic_info = feature_count(user_basic_info, [i, j])
    # 对数据进行特征组合以及
    ...
    #删除没有进行编码的特征
    user_basic_info.drop(['city', 'prodName', 'color', 'ct', 'carrier'], axis=1, inplace=True)
    # 读取用户的行为信息
