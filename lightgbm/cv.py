...
def app_cv(max_features, data_train, data_test):
    '''
    :param max_features:
    :return:
    '''
    ...
    #存储APP的名字(有效APP的名字)
    appNames.to_csv('effective_apps_max_features_5000.csv', header=False, index=False)
    sparse.save_npz("train_actived_cv_max_features_5000.npz", train_app_actived_cv)
    sparse.save_npz("test_actived_cv_max_features_5000.npz", test_app_actived_cv)
...
def app_actived_totalnum(active_sparse_matrix, effective_apps_file):
    ...
    effective_app.to_csv("effective_apps_actived_number_sorted.csv", index=False)
