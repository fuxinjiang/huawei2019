...
X_usage_time_new = sparse.load_npz(DATADIR_pro + 'usage/usageSum_tm_test.npz')

X_usage_duration_new = sparse.load_npz(DATADIR_pro + 'usage/usageSum_dur_test.npz')

X_constant = sparse.load_npz(DATADIR_pro + 'present_data/df_test_norm_sparse_876_10_constant.npz')
X_cv_max = sparse.load_npz(DATADIR_pro + 'present_data/test_actived_cv_max_features_5000.npz')
X_tfidf = sparse.load_npz(DATADIR_pro + 'tfidf/active_tf_idf_test.npz')
...
results = model.predict([X_app_6, X_bhv_log, X_bsc1, X_bsc2, X_act, X_app,  X_usage_time, X_usage_duration,
                         X_usage_time_new, X_usage_duration_new, X_tfidf, X_constant, X_cv_max ])
...
np.save('FCN_prob_6476.npy', proba_array_temp)
